...
% 迭代次数q[2,8]、记忆步长L[50,300]、分数阶阶次gam[0,2]、采样步长h[0,0.1]、滤波参数p[0,0.1]、b[1,10]、初始状态值x0[0,300]
%% 输入输出信号
load U1;
load Y1;
U1=U1;
...
for i=2:q
    for j=2:N
        w(i,j)=((1-(gam*(i-1)+1)/(j-1))*w(i,j-1)); %权值计算
    end
end
for i=2:q
    W1(i,:)=w(i,:)./h^((i-1)*gam); %微分算子，分数阶次为(i-1)*gam时 wL/T^((i-1)*gam)
end
w_w=w(2:q,N-L+1:N);
w_sum=sum(w_w,2);
w_ave=w_sum/L; %恒权重记忆法
for i=2:q
    for j=N-L+1:N
    w(i,j)=w_ave(i-1); %使得从最开始L步长以内的权值相等
    end
end
for i=2:q
    W2(i,:)=w(i,:)./h^((i-1)*gam); %微分算子，分数阶次为(i-1)*gam时 wL/T^((i-1)*gam)
end
%% 对滤波函数进行微分
if 2*L<N
    H1=zeros(N,2*L);
    for i=1:N
        for j=1:2*L
            if j<i
                ...
                H1(i,j)=f(j-i+1); %构造滤波函数矩阵，为了后面实现对滤波函数的微分。
    ...
    for i=1:N
        for j=1:N-2*L
            if j<i
                ...
                H2(i,j)=f(j-i+1); %构造滤波函数矩阵，为了后面实现对滤波函数的微分。
            end        %对一个采样点来说，采样时间最晚的微分算子和采样时间最早的滤波函数相乘，以此类推。
        end
    end
    G1=W1*H1; %对滤波函数进行微分
